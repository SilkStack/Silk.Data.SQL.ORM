...
{
	public class ExpressionConverter<TSource>
		where TSource : new()
	{
		public EntityModel<TSource> DataModel { get; }
		private ConverterVisitor _converterVisitor;

		public ExpressionConverter(EntityModel<TSource> dataModel)
		{
			DataModel = dataModel;
			_converterVisitor = new ConverterVisitor(dataModel);
		}

		public QueryExpression ConvertToCondition(Expression<Func<TSource, bool>> expression)
		{
		...
		private class ConverterVisitor : ExpressionVisitor
		{
			...
			private readonly EntityModel<TSource> _dataModel;

			public ConverterVisitor(EntityModel<TSource> dataModel)
			{
